...
fn update_spatial_hash(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    var hash_key = hash_position(pos.xy, half_boundries);
...
const hv1: u32 = 73856093;
const hv2: u32 = 19349663;
const hv3: u32 = 83492791; // for 3d hashing

fn get_shifted_2D_pos(pos: vec2<f32>, half_boundries: vec2<f32>) -> vec2<u32> {
    var floor_x = u32( floor( (pos.x + half_boundries.x) / SMOOTHING_RADIUS ) );
    var floor_y = u32( floor( (pos.y + half_boundries.y) / SMOOTHING_RADIUS ) );
    return vec2<u32>(floor_x, floor_y);
}


// seems funky to use a float as a hash value, but it works for now
fn hash_position(pos: vec2<f32>, half_boundries: vec2<f32>) -> u32 {
    var p = get_shifted_2D_pos(pos, half_boundries);
    // var hash = (p.x * hv1 + p.y * hv2); //if wrapping behavior isnt supported
    //seems to give more uniform distribution
    var hash = (p.x * hv1) ^ (p.y * hv2); //if wrapping behavior is supported
    return hash % max_particles;
}

// neighbor search
const neighbor_offsets_2D = array<vec2<i32>, 9>(
